...
@onready var mixer_sprite_2d: Sprite2D = $CanvasLayer/Control/ChemicalControl/MixerMaskSprite2D/MixerSprite2D
...
@onready var juice_buttons: Array[Button] = [
	alpha_button, gamma_button, beta_button,
	power_button, speed_button, durability_button,
	special_button, dexterity_button, guard_button]

@onready var tv_animated_sprite_2d: AnimatedSprite2D = $CanvasLayer/Control/TVControl/TVAnimatedSprite2D
...
@onready var start_tooltip_control: Control = $CanvasLayer/Control/ButtonControl/StartButton/StartTooltipControl
@onready var vial_rich_text_label: RichTextLabel = $CanvasLayer/Control/ChemicalControl/VialTooltipControl/VialRichTextLabel
...
func _process(_delta: float) -> void:
	...
	if juice_down and juice_timer.is_stopped():
		if mixer_dict.has(mixer_attribute.to_lower()):
			var total_cost: int = mixer_dict[mixer_attribute.to_lower()]["price"] * mixer_dict[mixer_attribute.to_lower()]["increase"]
			if GameGlobals.total_money >= total_cost:
				juice_timer.start()
				mixer_dict[mixer_attribute.to_lower()]["amount"] += 1
				GameGlobals.total_money -= total_cost
				
				if mixer_dict[mixer_attribute.to_lower()]["amount"] >= (mixer_dict[mixer_attribute.to_lower()]["increase"]*10):
				...
				if mixer_sprite_2d.modulate != Color(mixer_colors[mixer_attribute.to_lower()]):
					if not is_instance_valid(mixer_tween):
						...
						mixer_tween.tween_property(mixer_sprite_2d, "modulate", Color(mixer_colors[mixer_attribute.to_lower()]), 1)
					else:
						mixer_tween.kill()
						mixer_tween = get_tree().create_tween()
						mixer_tween.tween_property(mixer_sprite_2d, "modulate", Color(mixer_colors[mixer_attribute.to_lower()]), 1)
...
func drink_juice() -> void:
	for juice_attribute in mixer_dict.keys():
		...
		if juice_attribute != "alpha" and juice_attribute != "beta" and juice_attribute != "gamma":
			modifier = 5 * (float(mixer_dict[juice_attribute]["amount"])/10.0)
		else:
			modifier = GameGlobals.rng.randf_range(-10, 10) * (float(mixer_dict[juice_attribute]["amount"])/10.0)
